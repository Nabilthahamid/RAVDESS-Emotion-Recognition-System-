"""
ESRA-RELA++ V15-LoRA-Publication-Final: Publication-ready multimodal RAVDESS pipeline
====================================================================
...
FEATURE_CACHE_VERSION = "esra_rela_pp_v15_lora_final_1"
...
class _WaveDataset:
    """Minimal torch Dataset wrapper without importing torch at module load."""
    def __init__(self, waves: Sequence[np.ndarray], labels: Optional[Sequence[int]] = None):
...
def _train_lora_audio_model(
    ...
    train_loader = DataLoader(
        _WaveDataset([train_waves[int(i)] for i in train_positions], train_y[train_positions]),
    ...
    if len(val_positions) > 0:
        val_loader = DataLoader(
            _WaveDataset([train_waves[int(i)] for i in val_positions], train_y[val_positions]),
            batch_size=max(1, int(cfg.lora_batch_size)),
            shuffle=False,
            collate_fn=collate,
            num_workers=0,
